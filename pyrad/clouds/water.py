...
from numpy import append, copy, power, searchsorted, zeros

from .utils import CloudOptics, interp


class HuStamnes(CloudOptics):
    ...
    def optics(self, water_concentration, equivalent_radius, grid):
        ...
        extinction_coefficient = interp(self.bands, self.band_limits, beta, grid)
        single_scatter_albedo = interp(self.bands, self.band_limits, omega, grid)
        asymmetry_factor = interp(self.bands, self.band_limits, g, grid)
        return extinction_coefficient, single_scatter_albedo, asymmetry_factor


class Slingo(CloudOptics):
    """Liquid water cloud optics parameterization.
       doi: 10.1175/1520-0469(1989)046<1419:AGPFTS>2.0.CO;2

    Attributes:
        a: Optical depth coefficients [g-1 m2] (band).
        b: Optical depth coefficients [g-1 m2 micron] (band).
        band_limits: Lower/upper bounds of parameterization [cm-1].
        bands: Parameterization band centers [cm-1].
        c: Single-scatter albedo coefficients (band).
        d: Single-scatter albedo coefficients [micron-1] (band).
        e: Asymmetry factor coefficients (band).
        f: Asymmetry factor coefficients [micron-1] (band).
        max_radius: Maximum radius defined in parameterization [micron].
        min_radius: Minimum radius defined in parameterization [micron].
    """
    ...
    def __init__(self, path):
        with Dataset(path, "r") as dataset:
            self.max_radius = dataset.getncattr("max_radius")
            self.min_radius = dataset.getncattr("min_radius")
            band = dataset.variables["band"]
            self.band_limits = band.getncattr("valid_range")
            self.bands = copy(band[...])
    ...
    def optics(self, water_path, equivalent_radius, grid):
        """Calculates cloud optics.

        Args:
            water_path: Water path [g m-2].
            equivalent_radius: Droplet equivalent radius [micron].
            grid: Spectral grid [cm-1].

        Returns:
            optical_depth: Optical depth (grid).
            single_scatter_albedo: Single-scatter albedo (grid).
            asymmetry_factor: Asymmetry factor (grid).
        """
        r = max(self.min_radius, equivalent_radius) if equivalent_radius < self.max_radius \
        ...
        tau[1:n] = water_path*(self.a[:] + self.b[:]/r) #Equation 1.
        omega[1:n] = 1. - (self.c[:] + self.d[:]*r) #Equation 2.
        g[1:n] = self.e[:] + self.f[:]*r #Equation 3.
        tau[0], omega[0], g[0] = tau[1], omega[1], g[1]
        tau[-1], omega[-1], g[-1] = tau[-2], omega[-2], g[-2]

        optical_depth = interp(self.bands, self.band_limits, tau, grid)
        single_scatter_albedo = interp(self.bands, self.band_limits, omega, grid)
        asymmetry_factor = interp(self.bands, self.band_limits, g, grid)
